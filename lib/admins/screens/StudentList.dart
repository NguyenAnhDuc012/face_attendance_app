...
class _StudentListState extends State<StudentList> {
  final String _currentPage = 'Student'; // Thay đổi thành 'Student'
      ...
      body: Row(
        children: [
          ...
          Expanded(
            child: Column(
              children: [
                TopBar(title: 'Sinh viên', subtitle: 'Danh sách'), // Thay đổi title
                Expanded(
                  child: SingleChildScrollView(
                    padding: const EdgeInsets.all(24.0),
                    child: StudentTableCard(), // Thay đổi Widget
                  ),
                ),
                AppFooter(),
              ],
...
class StudentTableCard extends StatefulWidget {
  const StudentTableCard({Key? key}) : super(key: key);
...
class _StudentTableCardState extends State<StudentTableCard> {
  ...
  Future<void> _fetchData({int page = 1}) async {
    ...
    try {
      var result = await _studentService.fetchStudents(page: page);
      setState(() {
        _studentList = result['students'];
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                ElevatedButton.icon(
                  ...
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => const StudentCreate()), // Thay đổi
            ...
            _isLoading
              ...
              child: Padding(
                padding: EdgeInsets.all(32.0),
                child: Text('Không có dữ liệu sinh viên.'), // Thay đổi
              ),
            )
                : Column(
  ...
  DataRow _buildDataRow(Student student, int index) {
    ...
    return DataRow(
      ...
      cells: [
        ...
        DataCell(Text(student.studentClass?.name ?? '')), // Hiển thị tên lớp
        DataCell(Row(
          children: [
            IconButton(
              ...
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => StudentEdit(student: student)), // Thay đổi
            ...
            IconButton(
              ...
              tooltip: 'Xóa',
              onPressed: () => _handleDelete(student.id),
            ),
          ],
        )),
      ],
  ...
  Future<void> _handleDelete(int id) async {
    bool? confirm = await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Bạn có chắc chắn muốn xóa sinh viên này?'), // Thay đổi
        actions: [
    ...
    if (confirm == true) {
      try {
        await _studentService.deleteStudent(id); // Thay đổi
        _fetchData(page: _currentPage);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Xóa sinh viên thành công!'), backgroundColor: Colors.green), // Thay đổi
          );
        }
      } catch (e) {
        if (mounted) {
