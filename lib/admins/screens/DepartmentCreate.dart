...
class DepartmentCreate extends StatefulWidget {
  const DepartmentCreate({Key? key}) : super(key: key);
...
class _DepartmentCreateState extends State<DepartmentCreate> {
  ...
  Future<void> _loadFaculties() async {
    try {
      // Tải trang đầu tiên của danh sách khoa để hiển thị trong dropdown
      // Giả định rằng fetchFaculties trả về danh sách đầy đủ hoặc trang đầu tiên
    ...
    // TODO: Cân nhắc tải tất cả khoa nếu danh sách quá dài
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Tạo bộ môn thành công!'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Lỗi khi tạo bộ môn: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
    ...
    return Scaffold(
      body: Row(
        children: [
          ...
          ), // Highlight mục 'Department'
          Expanded(
            child: Column(
              children: [
                const TopBar(title: 'Bộ môn', subtitle: 'Thêm mới'),
                Expanded(
                  child: Center(
