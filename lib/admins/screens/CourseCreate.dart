...
class _CourseCreateState extends State<CourseCreate> {
  ...
  // Tải song song tất cả dữ liệu cho 4 dropdown
  Future<void> _loadDependencies() async {
    ...
    try {
      ...
      setState(() {
        ...
        if (_studentClasses.isNotEmpty) _selectedClass = _studentClasses.first;

        _studyPeriods = results[2]['studyPeriods'];
        if (_studyPeriods.isNotEmpty) _selectedPeriod = _studyPeriods.first;

        _lecturers = results[3]['lecturers'];
        if (_lecturers.isNotEmpty) _selectedLecturer = _lecturers.first;
      });
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Tạo lớp học phần thành công!'),
      ...
      body: Row(
        children: [
          ...
          Expanded(
            child: Column(
              children: [
                const TopBar(title: 'Lớp học phần', subtitle: 'Thêm mới'),
                Expanded(
                  child: Center(
