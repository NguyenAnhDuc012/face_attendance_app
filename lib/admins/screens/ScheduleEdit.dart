...
class ScheduleEdit extends StatefulWidget {
  ...
  const ScheduleEdit({Key? key, required this.schedule}) : super(key: key);
...
class _ScheduleEditState extends State<ScheduleEdit> {
  ...
  // Hàm helper chuyển "HH:mm" sang TimeOfDay
  ...
  // Tải dữ liệu cho 2 dropdown và gán giá trị cũ
  Future<void> _loadDependencies() async {
    ...
    try {
      ...
      setState(() {
        ...
        _selectedStartTime = _parseTime(widget.schedule.startTime);
        _selectedEndTime = _parseTime(widget.schedule.endTime);
  ...
  Future<void> _submit() async {
    ...
    try {
      await _scheduleService.updateSchedule(
          id: widget.schedule.id, // ID của TKB cần sửa
          courseId: _selectedCourse!.id,
          roomId: _selectedRoom!.id,
          dayOfWeek: _selectedDayOfWeek!,
          startTime: _formatTimeForService(_selectedStartTime!),
          endTime: _formatTimeForService(_selectedEndTime!)
      );

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Cập nhật TKB thành công! Các buổi học tương lai đã được tái tạo.'),
      ...
      body: Row(
        children: [
          ...
          Expanded(
            child: Column(
              children: [
                const TopBar(title: 'Thời khóa biểu', subtitle: 'Sửa'),
                Expanded(
                  child: Center(
